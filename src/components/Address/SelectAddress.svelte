<script>
  ...
  export let selectedAddress = undefined;
  ...
  $: if (mounted && $addressesQuery.data) {
    ...
    selectChoice = new Choices(select, {
      ...
      searchChoices: true,
      placeholder: true,
      placeholderValue: "Selecteer locatie",
      searchPlaceholderValue: "Zoek locatie..",
    ...
    selectChoice.setChoices(values, "value", "label", true);
    if (selectedAddress) {
      selectChoice.setChoiceByValue(selectedAddress._id);
    }
    mounted = false;
  }
...
<div class="tw-w-4/5 z-index-3">
  ...
  {/if}
  {#if $addressesQuery.data}
    <label class="tw-mt-6 tw-mb-2 tw-ml-1 tw-font-bold tw-text-xs tw-text-slate-700 dark:tw-text-white/80">{labelText}</label>
  {/if}
